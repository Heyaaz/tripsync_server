...
- 하지만 일정 조회/공개 공유 응답은 `place.name`, `address`, `placeName` 정도만 내려주고 있음 (`src/schedule/schedule.service.ts:256`, `src/schedule/schedule.service.ts:377` 부근)
- `GET /api/share/schedules/:scheduleId` 문서 예시의 `destination`, `tripDate`도 실제 응답에는 없음 (`docs/API_SPEC.md:859`, `src/schedule/schedule.service.ts:360`)
...
- `slots[].place.latitude`, `slots[].place.longitude` (지도 뷰와도 공용 가능)
- `share schedule` 응답의 `destination`, `tripDate`

### 완료 기준
- [ ] 일정 상세 응답에서 숨은 명소 배지 표시 가능한 필드 제공
- [ ] 공개 공유 응답에서 `destination`, `tripDate` 포함
...
1. **숨은 명소 배지/공유 응답 보강**
2. **TPTI 제출 이후 방 스냅샷/ready 상태 자동화**
3. **일정 생성 가드레일(락/재시도/만족도 기준)**
4. **TourAPI 자동 수집/보강 배치**
5. **운영 가드레일(rate limit / 로그 / env 검증)**
